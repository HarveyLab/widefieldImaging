...
function output = dftregistration(mov_fft, ref_ftt, ref_fftshift, ref_conj, usfac, maxAllowedShift)
...
CC = zeros(mlarge,nlarge, 'like', ref_fftshift);
...
[max1,loc1] = max(CC(maxAllowedShift, maxAllowedShift));
[~,loc2] = max(max1);
rloc=loc1(loc2);
cloc=loc2;
CCmax=CC(rloc,cloc);
